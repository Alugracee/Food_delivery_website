import React from 'react';
import "./exploreMenu.css";
import menu_list from "../../assets/assets.jsx"

const ExploreMenu = ({category, setCategory}) => {
  return (
    <div className="explore-menu" id="explore-menu">
      <h1>Explore our Menus</h1>
      <p className="explore-menu-text">Indulge in expertly crafted dishes designed to delight your palate. From savory appetizers to irresistible desserts, there's something for everyone.</p>
        <div className="menu-list">
            {menu_list.map((item, index) => (
              <div onClick={()=>setCategory(prev=>prev===item.menu_name?"All":item.menu_name)} className="menu-list-item" key={index}>                    
                <img src={item.menu_image} alt={item.menu_name} className={category===item.menu_name?"active":""} />
                <p>{item.menu_name}</p>
                    {/* <p>{item.menu_description}</p> */}
              </div>
            ))}
        </div>
        <hr />
      </div>
  )
}
export default ExploreMenu;
